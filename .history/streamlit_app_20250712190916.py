...
"""
streamlit frontend for Weather Forecast App
...
- Handles most API and user errors gracefully (invalid city, API key, etc).
...
if "csv_initialized" not in st.session_state:
    # Only clear CSV file the first time the app loads (per browser session)
...
# "Get Forecast" button triggers all main logic below
if get_clicked:
    all_cities_entered = [c.strip() for c in city_input.split(",") if c.strip()]

    # If user enters >3, warn and use first 3 only
    ...
    for city in cities:
        try:
            ...
            for day in five_day:
                ...
                # table_data is list of lists for st.table
                table_data.append(
                    [day["date"],
                     f'{day["temp"]}°F',
            ...
            # Show as table. Use dict-of-lists for labeled columns.
            st.table(
            ...
            if date_labels is None:
                date_labels = [d["date"] for d in five_day]  # set once

        except requests.exceptions.HTTPError as e:
            code = e.response.status_code
            # Streamlit-style errors for common HTTP issues
            if code == 404:
    ...
    if city_names:
        ...
        # File download button for session CSV
        ...
        # Find hottest and coldest day among all cities and days
